...
def add_path(old_paths):
    # return [os.path.join('./ptb', s) for s in old_paths]
    return old_paths
...
if __name__ == '__main__':
    ...
    info['word_emb_d%d' % emb_arr.shape[1]] = 'data/processed/pos/word_emb_d%d.npy' % emb_arr.shape[1]
    ...
    for n in [len(train_wod_list)//50, len(train_wod_list)//10, len(train_wod_list)]:
        file_name = wb.mkdir('data/processed/pos/') + 'train.%d' % n
    ...
    #     np.random.seed(seed)
    #     inds = np.random.permutation(len(train_wod_list))
    #     write_txt('data/processed/pos/train.%d.seed.%d.wod'%(n,seed), [train_wod_list[i] for i in inds[:n]])
    #     write_txt('data/processed/pos/train.%d.seed.%d.tag'%(n,seed), [train_tag_list[i] for i in inds[:n]])


    import pickle
    pickle.dump(id2tag,open('data/processed/pos/id2tag.pkl','wb'))
    info['id2tag']='data/processed/pos/id2tag.pkl'

    with open('data/processed/pos/data.info', 'wt') as f:
