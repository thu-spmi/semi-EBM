...
if __name__ == '__main__':
    wb.mkdir('data/processed/chunk/')
    with open('data/raw/chunk/data.info') as f:
        info = json.load(f)
    ...
    info['word_emb_d%d' % emb_arr.shape[1]] = 'data/processed/chunk/word_emb_d%d.npy' % emb_arr.shape[1]
    ...
    for n in [len(train_wod_list)//50, len(train_wod_list)//10, len(train_wod_list)]:
        file_name = wb.mkdir('data/processed/chunk/') + 'train.%d' % n
    ...
    #     np.random.seed(seed)
    #     inds = np.random.permutation(len(train_wod_list))
    #     write_txt('data/processed/chunk/train.%d.seed.%d.wod' % (n, seed), [train_wod_list[i] for i in inds[:n]])
    #     write_txt('data/processed/chunk/train.%d.seed.%d.tag' % (n, seed), [train_tag_list[i] for i in inds[:n]])
    #     info['train%d_seed%d' % (n, seed)] = [('data/processed/chunk/train.%d.seed.%d.wod' % (n, seed), 'data/processed/chunk/train.%d.seed.%d.tag' % (n, seed))]
    import pickle
    pickle.dump(id2tag,open('data/processed/chunk/id2tag.pkl','wb'))
    info['id2tag']='data/processed/chunk/id2tag.pkl'

    with open('data/processed/chunk/data.info', 'wt') as f:
