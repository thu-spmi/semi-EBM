...
train_RF = th.function(inputs=[x_revised,x_unl,lr,base_RF_loss_weight,potential_control_weight], outputs=loss_unl, updates=RF_param_updates)
#weight norm initalization
init_param = th.function(inputs=[h,x_unl], outputs=None, updates=init_updates)
...
for epoch in range(args.max_e):
    ...
    if epoch==0:
        init_param(np.cast[th.config.floatX](np.random.uniform(size=(500,100))),trainx_unl[:500]) # data based initialization
        if args.load:
    ...
    for t in range(nr_batches_train):
        ...
        # updata random field
        lo_unl = train_RF(x_revised, trainx_unl[ran_from:ran_to], lr_D, base_RF_loss_weight,
    ...
    # test
    print(
        "epoch %d, time = %ds, loss_unl = %.4f" % (
            epoch + 1, time.time() - begin, loss_unl))
    sys.stdout.flush()
    ...
    if (epoch + 1) % 50 == 0:
        ...
        save_weights('cifar_model/pretrain_nrf_ep%d.npy' % (epoch + 1), params)
