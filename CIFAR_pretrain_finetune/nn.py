...
def adam_updates(params, cost,c_c=None, lr=0.001, mom1=0.9, mom2=0.999):
    updates = []
    grads = T.grad(cost, params,c_c)
...
class MLPConcatLayer(lasagne.layers.MergeLayer):
    ...
    '''
    def __init__(self, incomings, num_cls=10, **kwargs):
    ...
    def get_output_for(self, input, init=False, deterministic=False, **kwargs):
        ...
        if init:
            ...
            try:
                ...
                self.init_updates = [(self.weight_scale, self.weight_scale / stdv)]
        else:
            try:
