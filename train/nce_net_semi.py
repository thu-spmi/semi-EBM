...
# Apache 2.0.
# This script contrains net structures of TRF semi-supervised (JRF) training experiments.
...
class Net(object):
    ...
    def create_net(self, inputs, labels, lengths, word_to_chars):
        ...
        # for TRF
        self.trf_phi, self.trf_logz = self.create_trf_phi(rnn_outputs, emb, lengths, self.crf_logz,crf_weight=self.config.crf_weight,trf_weight=self.config.trf_weight)
    ...
    def create_crf_phi(self, rnn_outputs, labels, lengths):
        ...
        if self.config.std:
            ...
            logz = tf.contrib.crf.crf_log_norm(outputs[:, 1:-1], lengths - 2, edge_matrix)
            return log_likelihood,logz
    ...
    def create_trf_phi(self, rnn_outputs, emb, lengths, crf_logz,crf_weight=1,trf_weight=0):

        phi = crf_logz*crf_weight  # of shape [batch_size]
        if trf_weight!=0:
            ...
            phi += tf.reduce_sum(outputs, axis=-1)*trf_weight  # of shape [batch_size]
        # logz
...
class SemiNet(object):
    def __init__(self, config, is_training=True, device='/gpu:0', name='uns-net', word_to_chars=None):
        ...
        self.crf_net = Net(config, is_training=True, device=device, name=name, reuse=None,
                           word_to_chars=word_to_chars)
        ...
        if is_training:
            ...
            self.loss = self.trf_net.nce_loss *  self.alpha + self.crf_net.crf_loss
    ...
    def run_update(self, session,
                   ...
                   learning_rate=1.0,
                   max_batch_size=100,alpha=0.1):

        session.run(self.grad_clean)

        crf_size = crf_inputs.shape[0]
        trf_size = data_num
        batch_num = max(max(crf_size, trf_size) // max_batch_size, 1)
        ...
        for i in range(batch_num):
            session.run(self.grad_update,
                        {self.crf_net._inputs: crf_inputs[i*crf_batch_size: (i+1)*crf_batch_size],
                         ...
                         self.trf_net._dropout: self.config.trf_dropout,
                         self.alpha:alpha
                        })
