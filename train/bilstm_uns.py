...
# Apache 2.0.
# This script contrains net structures of biLSTMLM unsupervised training experiments.
...
class Net(object):
    ...
    def create_net(self, inputs, labels, lengths, word_to_chars):
        ...
        self.create_loss(rnn_outputs,emb,lengths,inputs)

    def create_loss(self, rnn_outputs, emb, lengths,inputs):
        ...
        # add a linear layers
        outputs_fw = layers.linear(outputs_fw, self.config.vocab_size, activate=None, name='trf_output_layer_fw')
        outputs_bw = layers.linear(outputs_bw, self.config.vocab_size, activate=None, name='trf_output_layer_bw')
        loss_forward=tf.nn.sparse_softmax_cross_entropy_with_logits(logits=outputs_fw[:, 0:-1],labels=inputs[:,1:])
        loss_backward=tf.nn.sparse_softmax_cross_entropy_with_logits(logits=outputs_bw[:, 1:] , labels=inputs[:, 0:-1])

        self.nce_loss=tf.reduce_mean(tf.reduce_sum((loss_forward+loss_backward)*tf.sequence_mask(lengths - 1, maxlen=tf.shape(emb)[1] - 1, dtype=tf.float32),1))
    ...
    def run_trf_logp(self, session, inputs, lengths):
        res=session.run(self.nce_loss, {self._inputs: inputs, self._lengths: lengths})
        return res

    def run_trf_update(self, session, inputs, lengths, data_num, learning_rate=1.0):
...
class UnsNet(object):
    def __init__(self, config, is_training=True, device='/gpu:0', name='uns-net', word_to_chars=None):
        self.config = config
    ...
    def run_trf_logp(self, session, inputs, lengths):
        res=self.trf_net.run_trf_logp(session, inputs, lengths)
        return res
    ...
    def run_update(self, session,
        ...
        trf_size = data_num
        batch_num = trf_size // max_batch_size
